  ...
  if (selectedId && selectedId.startsWith("menulist ")) {
    ...
    if (m?.message?.templateButtonReplyMessage) {
      ...
      if (id?.startsWith("menulist ")) {
        ...
        if (!plugins.length)
          return this.reply(m.chat, "Tidak ada perintah di kategori ini.", m);
        // const list = plugins
        //   .map((p) => `⫸ ${global.prefix || "/"}${p.help[0]}`)
        //   .join("\n");
        const cleanHelp = (cmd) =>
          cmd
            .replace(/^\/\^.*?\$\/[gimuy]*\//, "")
    ...
    const user =
      (m.isGroup
        ? participants.find((u) => {
            ...
            const pn = u.phoneNumber
              ? String(u.phoneNumber).replace(/[^0-9]/g, "") + "@s.whatsapp.net"
              : null;
      ...
      if ((usedPrefix = (match[0] || "")[0])) {
        ...
        if (requiredLimit > 0) {
          ...
          if (currentLimit < requiredLimit) {
            this.reply(
              m.chat,
              "Limit kamu kurang,\nketik `.premium` untuk membeli Role Premium",
              m
            );
            continue;
          }
    ...
    case "remove":
      if (chat.welcome) {
        ...
        for (let user of participants) {
          ...
          let userJid =
            this.getJid(String(user).decodeJid()) || String(user).decodeJid();
          let pp;
          try {
            const pps = await this.profilePictureUrl(userJid, "image").catch(
          ...
          const welcomeBg =
            "https://telegra.ph/file/666ccbfc3201704454ba5.jpg ";
          ...
          const wel = `${
            APIs.ryzumi
          }/api/image/welcome?username=${encodeURIComponent(
            username
          )}&group=${encodeURIComponent(gcname)}&avatar=${encodeURIComponent(
            pp || ""
          )}&bg=${encodeURIComponent(welcomeBg)}&member=${gcMem}`;
          const lea = `${
            APIs.ryzumi
          }/api/image/leave?username=${encodeURIComponent(
            username
          )}&group=${encodeURIComponent(gcname)}&avatar=${encodeURIComponent(
            pp || ""
          )}&bg=${encodeURIComponent(leaveBg)}&member=${gcMem}`;
...
global.dfail = async (type, m, conn) => {
  try {
    const msg = {
      ...
      botAdmin:
        "*ONLY BOT ADMIN* • CMD INI HANYA BISA DIGUNAKAN KETIKA BOT MENJADI ADMIN",
      unreg:
        "*YOU ARE NOT REGISTERED YET* • KETIK .register nama.umur\nCONTOH kairi.20",
